...
管理后台位于 `/admin`。数据填充器会创建默认银行转账渠道，但不会创建示例商品。
...
人工付款时，必须先在供应商后台打开该操作对应的准确账单，对照冻结操作和本地账单/服务核对账单 ID、商品、金额及币种，再支付该账单。供应商侧显示已付款后，还要确认主机确实属于同一账单，然后在 **管理后台 > 供应商操作 > 已在上游人工付款并确认主机** 中填写主机 ID 并勾选明确作证。这只会记录具名人工证据，不是密码学付款证明；该操作不会调用供应商付款接口，也不会直接激活服务。只有后续只读轮询返回 `Active` 才能激活。单独使用主机证据关联不构成付款确认。

管理员可为单个供应商显式启用高级兼容选项，以允许旧版 `/apply_credit`。启用或关闭都使用共享的供应商敏感操作限流，并且在该账户存在未终结操作或待结算订单项路由时禁止变更；仅凭据轮换的例外不适用于此选项。审计只记录该选项变更前后的布尔值。旧版接口无法携带预期金额、币种、账单版本或幂等前置条件，因此存在检查与使用时点分离（TOCTOU）及无上限扣款风险：账单可能在报价校验后、余额扣除前发生变化，冻结报价无法原子限制供应商实际扣除的金额或币种。显式启用后，只有结构有效的应用状态 `1001` 才构成持久化付款确认；状态 `200` 和未知结果仍视为未确认，且绝不重放。
...
活动供应商账户即使存在未终结操作，也可以仅轮换已保存凭据。账户 ID 和冻结的基础地址身份保持不变，供应商 JWT 缓存会失效，已有排队操作将使用新凭据。基础地址、驱动、内部代码、启停状态、映射和旧版余额兼容选项仍须等相关操作终结后才能修改。定时遗留续费协调器会把预发布版本留下的排队 `renew` 记录以 `unsupported_supplier_renewal` 安全终结，且不扣款、不访问供应商。
